...
class Ratio:
    ...
    def Annual_Return(self):
        return (self.data.dropna().cumsum()[-1]+1)**(1/(len(self.data)/250))-1
    ...
    def Sharpe(self):
        rf = 0.000005
        return (self.data.mean() - rf) / self.data.std()

    def Sortino(self):
        rf = 0.000005
        return (self.data.mean() - rf) / self.data[self.data < 0].std()
...
class Optimization_Map:
    ...
    def CumRet(self, xfrom, xto, xby, yfrom, yto, yby):
        for i in range(xfrom, xto, xby):
            for j in range(yfrom, yto, yby):
                I = i / 100
                J = j / 100
...
if __name__ == '__main__':
    ...
    mode = 'o'
    bands = BBands(stock, mode, 20, 2, 2)
    ...
    print('年化波動率：', Ratio(return_data['return_a']).Annual_Volatility())
    ...
    print('年化波動率：', Ratio(return_data['d_ret']).Annual_Volatility())
    ...
    Optimization_Map(mode).CumRet(25, 300, 25, 25, 300, 25)
    Optimization_Map(mode).RonMDD(25, 300, 25, 25, 300, 25)
    Optimization_Map(mode).Sharpe_Ratio(25, 300, 25, 25, 300, 25)
    Optimization_Map(mode).Sortino_Ratio(25, 300, 25, 25, 300, 25)
